...
async function main() {
  ...
  const v = produits.find((p) => norm(p.nom) === norm("Table basse"));
  if (!v) {
    console.error(`⚠ Produit "Table basse" introuvable dans la gamme "${gamme.nom}".`);
  ...
  await prisma.produitVitrine.update({
    where: { id: v.id },
    data: { axesDeclinaisons: [], declinaisons: [ mk({}, 335, "BU55") ] },
  });
