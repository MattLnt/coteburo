...
async function main() {
  ...
  await prisma.produitVitrine.update({ where: { id: v.id }, data: {
    axesDeclinaisons: [ { id: "structure", nom: "Structure", valeurs: ["Gris foncé","Gris clair"] } ],
    declinaisons: [
      mk({ structure: "Gris foncé" }, 295, "LAN01GFGR"),
      mk({ structure: "Gris clair" }, 325, "LAN02GMGR"),
    ],
  } });
  await prisma.groupeFinition.deleteMany({ where: { vitrineId: v.id } });
  console.log(`  ✓ ${gamme.nom} / ${v.nom} — 2 combinaisons (Gris foncé 295€ / Gris clair 325€)`);
  console.log(`\n✓ LANDO traitée.`);
}
