...
const NOM_GAMME = "KHONG";
const NOM_PRODUIT = "Chaise";

const COLORIS = [
  { nom: "Bleu", couleur: "#3f6fa3", ordre: 0 },
  { nom: "Gris blanc", couleur: "#d5d5d2", ordre: 1 },
];
...
async function main() {
  ...
  await prisma.produitVitrine.update({ where: { id: v.id }, data: {
    axesDeclinaisons: [ { id: "version", nom: "Version", valeurs: ["Standard","Papillon"] } ],
    declinaisons: [
      mk({ version: "Standard" }, 275, "KHG02.."),
      mk({ version: "Papillon" }, 275, "KHG03BL"),
    ],
  } });
  ...
  console.log(`  ✓ ${gamme.nom} / ${v.nom} — 2 combinaisons (275€) + coloris`);
  console.log(`\n✓ KHONG traitée.`);
}
