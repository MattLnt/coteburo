...
async function main() {
  const vitrine = await prisma.produitVitrine.findFirst({
    where: { nom: "Plan compact", gamme: { nom: "Partage" } },
  });

  if (!vitrine) {
    console.error('Produit introuvable — vérifie que le nom est bien exactement "Plan compact" dans la gamme "Partage".');
    process.exit(1);
  }

  // ─── 1. Déclinaisons + prix + références ───
  // Même prix entre Droit/Gauche pour une même longueur, mais référence différente
  // → gardé en axe de Déclinaison pour que la bonne réf suive.
  const axeLongueur = { id: "longueur", nom: "Longueur", valeurs: ["160", "180"] };
  const axeSens = { id: "sens", nom: "Sens du retour", valeurs: ["Droit", "Gauche"] };

  const declinaisons = [
    { id: uid(), valeurs: { longueur: "160", sens: "Droit" }, prixTarifHT: "520", prixVenteHT: "", prixVerrouille: false, referenceFournisseur: "BX72" },
    { id: uid(), valeurs: { longueur: "160", sens: "Gauche" }, prixTarifHT: "520", prixVenteHT: "", prixVerrouille: false, referenceFournisseur: "BX73" },
    { id: uid(), valeurs: { longueur: "180", sens: "Droit" }, prixTarifHT: "540", prixVenteHT: "", prixVerrouille: false, referenceFournisseur: "BX74" },
    { id: uid(), valeurs: { longueur: "180", sens: "Gauche" }, prixTarifHT: "540", prixVenteHT: "", prixVerrouille: false, referenceFournisseur: "BX75" },
  ];
