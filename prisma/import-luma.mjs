...
const NOM_GAMME = "Luma";
const NOM_PRODUIT = "Fauteuil visiteur";
...
async function main() {
  ...
  const decl = [
      mk({coloris:"Noir (résille noire)"},369,"LM05/N1"),
      mk({coloris:"Blanc (résille grise)"},394,"LM05/B1"),
  ];
  ...
  await prisma.produitVitrine.update({
    ...
    data: {
      axesDeclinaisons: [
        { id: "coloris", nom: "Coloris", valeurs: ["Noir (résille noire)","Blanc (résille grise)"] },
      ],
      declinaisons: decl,
    },
  });
  await prisma.groupeFinition.deleteMany({ where: { vitrineId: v.id } });
  console.log(`  ✓ ${gamme.nom} / ${v.nom} — ${decl.length} combinaisons (coloris = axe de prix, pas de finition séparée)`);
  console.log(`\n✓ Luma traitée.`);
}
