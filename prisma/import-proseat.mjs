...
async function main() {
  ...
  await prisma.produitVitrine.update({ where: { id: v.id }, data: {
    axesDeclinaisons: [ { id: "version", nom: "Version", valeurs: ["Tout maille","Maille et tissu"] } ],
    declinaisons: [
      mk({ version: "Tout maille" }, 395, "PROS01NR"),
      mk({ version: "Maille et tissu" }, 395, "PROS02NR"),
    ],
  } });
  await prisma.groupeFinition.deleteMany({ where: { vitrineId: v.id } });
  console.log(`  ✓ ${gamme.nom} / ${v.nom} — 2 combinaisons (395€, Noir)`);
  console.log(`\n✓ PROSEAT traitée.`);
}
