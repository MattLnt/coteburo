...
/**
 * La valeur qui compte pour un choix : celle que le client a retenue, ou —
 * s'il n'a pas répondu et que le tarif ne laisse qu'une possibilité — celle-là.
 *
 * Une étape de finition qui n'a plus qu'une valeur ouverte n'est jamais posée :
 * la poser ferait perdre un clic pour rien. Mais elle doit tout de même entrer
 * dans la référence. Chez Eko, page 262, le socle MÉTAL porte sa couleur dans
 * sa référence — DY195G est noir — et sa colonne « finition socle » est vide ;
 * la serrure À CLÉ n'a pas de finition non plus. Les deux reçoivent une valeur
 * « — » au jeton vide, que les exclusions laissent seule ouverte. Sans cette
 * reprise, la question disparaissait de l'écran ET de la référence, et la
 * fiche se déclarait incommandable sans rien avoir à se reprocher.
 */
function valeurEffective(produit, choix, reponses) {
  const retenue = valeurRetenue(choix, reponses);
  if (retenue) return retenue;
  if (choix?.nature !== "finition") return null;
  const ouvertes = valeursFinitionOuvertes(produit, choix, reponses);
  return ouvertes.length === 1 ? ouvertes[0] : null;
}
...
export function assemblerReference(produit, reponses = {}) {
  ...
  for (const choix of participants) {
    const valeur = valeurEffective(produit, choix, reponses);
...
//
// LA QUANTITÉ
//   Un élément ne vaut pas toujours un exemplaire. Chez Eko, page 263, les
//   portes se vendent par kit de trois ou de quatre, et il en faut deux pour
//   un casier six cases, trois pour un neuf cases. L'élément porte donc une
//   quantité, et le prix de la ligne la multiplie. Absente, elle vaut un —
//   le cas de tous les éléments de Quiétude.
...
export function elementsDe(produit, reponses = {}) {
  ...
  return liste.map((el) => {
    ...
    return {
      ...
      reference: ref,
      quantite,
      // Le prix UNITAIRE, tel que le tarif l'écrit, et le total de la ligne.
      // Les deux, parce que la fiche montre « 3 × 96,00 € » et additionne
      // 288,00 € : afficher l'un sans l'autre force chacun à recalculer.
      prixTarifHT: el.prixTarifHT ?? null,
      prixTotalHT: el.prixTarifHT == null ? null : el.prixTarifHT * quantite,
      ecoContribution: (el.ecoContribution ?? 0) * quantite,
      jetons,
    };
  });
}
...
export function commandable(produit, reponses = {}) {
  ...
  // Un produit VENDU SUR DEVIS n'a pas de prix, et c'est normal : les cabines
  // acoustiques Essentielle et les tisaneries Oasys se chiffrent au cas par
  // cas. Exiger un prix ici désactivait leur bouton — qui affichait « prix
  // absent du tarif » au lieu de « Demander un devis », et le client ne
  // pouvait même plus demander le devis.
  const surDevis = !!(produit?.gamme?.venteSurDevis || produit?.venteSurDevis);
  if (!surDevis && comb.prixTarifHT == null) return { ok: false, motif: "prix absent du tarif" };
