...
export async function getFavorisContext() {
  ...
  if (!session?.user?.id) return { favorisCodes: [], favorisVitrines: [], connecte: false, estAdmin: false };
  ...
  return {
    ...
    connecte: true,
    // Le catalogue montre à l'admin un filtre favoris / non favoris : ses
    // favoris lui servent de pointage pendant le travail sur les visuels.
