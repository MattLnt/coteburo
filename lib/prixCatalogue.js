...
export function prixVitrine(vitrine, {
  declinaisonId = null, combinaison = null, surDevis = false, marge = 0,
} = {}) {
  if (!vitrine) {
  ...
  if (combinaison) {
    // La variante vivante l'emporte sur le champ JSON d'avant, que la
    // migration a laissé en place sans le maintenir : un prix corrigé dans
    // l'administration écrit la combinaison, jamais ce JSON, et la commande
    // facturait donc le montant figé au jour de la reprise.
    base = prixLigne(combinaison, marge);
    if (base == null) motif = "aucun tarif sur cette variante";
  } else if (vitrine.sansDeclinaisons || !declinaisonId) {
