...
export function appliquerPromoVitrine(vitrine, prixBase) {
  ...
  if (!pct || prixBase == null || prixBase < 0) {
    return { enPromo: false, prixFinal: prixBase, prixBase, promoPct: null };
  }
  const now = new Date();
  ...
  return { enPromo: true, prixFinal: arrondir2(prixBase * (1 - pct / 100)), prixBase, promoPct: pct };
}
...
export function prixPublicsVitrine(vitrine, { surDevis = false, marge = 0 } = {}) {
  ...
  const declinaisons = vitrine.sansDeclinaisons
    ...
    : (Array.isArray(vitrine.declinaisons) ? vitrine.declinaisons : []).map((d) => {
        const promo = appliquerPromoVitrine(vitrine, prixLigne(d, marge));
        return {
          id: d.id,
          valeurs: d.valeurs,
