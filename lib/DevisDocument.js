...
const s = StyleSheet.create({
  ...
  cRef: { width: "10%" },
  cDesc: { width: "40%" },
  cQte: { width: "9%", textAlign: "right" },
  cPU: { width: "11%", textAlign: "right" },
  cEco: { width: "10%", textAlign: "right" },
  ...
  // ── Éco-contribution ──
  ecoNote: { marginTop: 10, fontSize: 7, color: C.soft, lineHeight: 1.5 },
...
export function DevisDocument({ d, reglages = {} }) {
  ...
  // L'éco-contribution est une taxe de recyclage que Côté BURO paie au
  // fabricant et refacture à l'identique. Le code de l'environnement
  // impose qu'elle figure distinctement sur le document, sans marge.
  const totalEco = d.totalEcoPart != null
    ? d.totalEcoPart
      ...
      <Page size="A4" style={s.page}>
        ...
        {lignes.map((l, i) => (
          <View key={i} style={[s.tr, i === lignes.length - 1 ? s.filetBas : null]} wrap={false}>
            ...
            {avecEco ? (
              <Text style={[s.td, s.cEco, { color: C.soft }]}>
        ...
        <View style={s.basPage}>
          ...
          <View style={{ width: 210 }}>
            <View style={{ borderWidth: 1, borderColor: C.line }}>
            ...
            {avecEco ? (
              <Text style={s.ecoNote}>
                L&apos;éco-participation est une contribution obligatoire au recyclage
                du mobilier professionnel (article L541-10 du code de l&apos;environnement).
          ...
          <View style={s.blocTotaux}>
            ...
            {avecEco ? (
              <View style={s.totalLigne}>
                <Text>Éco-participation</Text><Text>{euro(totalEco)}</Text>
              </View>
            ) : null}
            {d.fraisLivraison > 0 ? (
