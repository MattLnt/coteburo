...
const FORME = {
  ...
  bestSeller: true,
  enAvant: true,
  // Les rayons ne servent qu'à l'administration — le site entre par l'URL du
  // rayon, pas par la fiche — mais ils tiennent en une jointure et évitent
  // une seconde requête sur un écran qui en fait déjà quatre.
  ...
  choix: {
    ...
    select: {
      ...
      ordre: true, rangReference: true, obligatoire: true, element: true,
      valeurs: {
        orderBy: { ordre: "asc" },
        select: {
  ...
  combinaisons: {
    select: {
      ...
      // Les éléments d'une fiche composée. Sans eux, estComposee() rend faux
      // et la fiche retombe sur la référence unique — en concaténant les
      // jetons des quatre éléments sur la référence du premier.
      elements: true,
    },
  },
...
function heriter(produit) {
  ...
  for (const choix of produit.choix || []) {
    for (const valeur of choix.valeurs || []) {
      // L'administration doit pouvoir distinguer une couleur propre à la
      // valeur d'une couleur reçue du nuancier : sans ce drapeau, l'écran
      // montrerait la teinte héritée dans le champ de surcharge, et la
      // première modification l'y figerait.
      valeur.heritee = false;
      if (valeur.modele) {
        if (!valeur.couleur) { valeur.couleur = valeur.modele.couleur; valeur.heritee = true; }
        if (!valeur.imageUrl) { valeur.imageUrl = valeur.modele.imageUrl; valeur.heritee = true; }
      }
