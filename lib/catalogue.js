  ...
  const categories = await prisma.categorie.findMany({
    ...
    include: {
      sousCategories: {
        ...
        // L icône du rayon suit : le menu la montre devant son nom, et
        // retombe sur celle de la catégorie quand le rayon n en a pas.
        select: { nom: true, slug: true, icone: true },
      },
      // Une catégorie sans rayon n'est pas forcément vide — Cabines en est
      // une. Le menu a besoin du compte pour ne pas annoncer des nouveautés
      // à venir devant cinq fiches en vente.
      _count: { select: { vitrines: { where: { publie: true, accessoireSeul: false } } } },
    },
  });
...
// Les fiches publiées, sous la forme minimale qu'attend le plan de site.
//
// La requête est volontairement maigre : getCartesFiltrables charge les prix,
// les marges et les campagnes, dont un plan de site n'a que faire.
//
// La catégorie principale décide de l'URL canonique, et c'est la même règle
// qui vaut ici que sur la fiche : /[categorie]/[[...rest]] renvoie un 404
// quand la sous-catégorie de l'URL n'est pas celle du produit, justement pour
// qu'une fiche n'ait pas deux adresses valides. Le plan de site annoncerait
// sinon des URL introuvables.
export async function getUrlsCatalogue() {
  ...
  return vitrines
    ...
    // Une fiche sans catégorie n'a pas d'URL à elle : urlProduit renvoie alors
    // /catalogue, qui est déjà dans le plan. On l'écarte plutôt que de la
    // répéter.
    .filter((e) => e.url !== "/catalogue");
}
...
// Le tri par défaut : les cartes mises en avant pour la cible ouverte
// (le rayon s'il y en a un, sinon la catégorie, sinon le catalogue entier),
// dans l'ordre de l'admin, puis les autres par nom.
export function trieurMiseEnAvant({ categorieSlug, sousCategorieSlug, parNom }) {
  const rang = sousCategorieSlug ? (c) => c.avantRayon
    : categorieSlug ? (c) => c.avantCategorie
      : (c) => c.avantCatalogue;
  return (a, b) => {
...
export function clePrixLigne(vitrineId, declinaisonId, combinaisonId, elementCle) {
  // La combinaison et l'élément entrent dans la clé : sans eux, les quatre
  // lignes d'une fiche composée — qui partagent la fiche ET la combinaison —
  // se confondraient et recevraient toutes le même prix.
  return `${vitrineId || ""}::${declinaisonId || ""}::${combinaisonId || ""}::${elementCle || ""}`;
}
...
export async function prixDepuisBase(lignes) {
  ...
  // Les combinaisons citées par ces lignes, pour les fiches composées : la
  // ligne d'un élément se chiffre au prix de SON élément, pas à celui de la
  // composition entière.
  const combiIds = [...new Set((lignes || []).map((l) => l.combinaisonId).filter(Boolean))];
  const combinaisons = combiIds.length
    ? await prisma.combinaison.findMany({
      where: { id: { in: combiIds } },
      // Combinaison ne porte NI prixVenteHT NI prixVerrouille — le prix de
      // vente se recalcule, il ne se stocke pas. Les demander ici faisait
      // échouer toute demande de devis citant une combinaison.
      select: { id: true, prixTarifHT: true, ecoContribution: true, referenceBase: true, elements: true },
    })
    : [];
  const parCombi = new Map(combinaisons.map((k) => [k.id, k]));

  for (const l of lignes) {
    if (!l.vitrineId) continue;
    const cle = clePrixLigne(l.vitrineId, l.declinaisonId, l.combinaisonId, l.elementCle);
    if (resultats.has(cle)) continue;

    const v = parId.get(l.vitrineId);
    const surDevis = v ? v.gamme.venteSurDevis || v.venteSurDevis : false;

    const k = l.combinaisonId ? parCombi.get(l.combinaisonId) : null;
    const element = l.elementCle && Array.isArray(k?.elements)
      ? k.elements.find((e) => e.cle === l.elementCle)
      : null;
    if (element) {
      resultats.set(cle, prixVitrine(v, {
        combinaison: { ...k, prixTarifHT: element.prixTarifHT, prixVenteHT: null, prixVerrouille: false },
        surDevis, marge,
      }));
      continue;
    }
    if (k) { resultats.set(cle, prixVitrine(v, { combinaison: k, surDevis, marge })); continue; }
...
export async function getCarteFront(gammeSlug, carteSlug) {
  ...
  // « Vous aimerez aussi » : les produits choisis dans l'admin s'ils existent,
  // sinon une suggestion automatique — même gamme d'abord, puis même
  // sous-catégorie. Six au plus, contre huit et la catégorie entière avant.
  const inclureCarte = {
  ...
  const choisis = (vitrine.produitsLies || []).map((p) => p.id);
  let autresRaw = [];

  if (choisis.length > 0) {
    const trouves = await prisma.produitVitrine.findMany({
      // Même règle que le catalogue et la recherche : un produit qui ne se
      // vend qu'AVEC un autre n'est pas une suggestion. L'alcôve Quiétude
      // réapparaissait ici sous la fiche composée qui la remplace, achetable
      // d'un clic — exactement ce qu'accessoireSeul devait empêcher.
      where: { id: { in: choisis }, publie: true, accessoireSeul: false, gamme: { publie: true } },
      include: inclureCarte,
    });
    // On respecte l'ordre de saisie de l'admin, que findMany ne garantit pas.
    const parId = new Map(trouves.map((v) => [v.id, v]));
    autresRaw = choisis.map((id) => parId.get(id)).filter(Boolean);
  } else if (categorieSlug) {
    const candidats = await prisma.produitVitrine.findMany({
      where: {
        publie: true,
        ...
        gamme: { publie: true },
        OR: [
          { gammeId: vitrine.gammeId },
          ...(sousCategorieSlug ? [{ sousCategories: { some: { slug: sousCategorieSlug } } }] : []),
          { categories: { some: { slug: categorieSlug } } },
        ],
        // Un accessoire n'est pas une suggestion : il se vend AVEC le produit,
        // pas à la place.
    ...
    const rang = (v) => {
      if (v.gammeId === vitrine.gammeId) return 0;
      if (sousCategorieSlug && v.sousCategories.some((s) => s.slug === sousCategorieSlug)) return 1;
      return 2;
    };
    autresRaw = [...candidats].sort((a, b) => rang(a) - rang(b)).slice(0, 6);
  }
...
export async function getCartesFiltrables({
  ...
  const where = {
    publie: true,
    // Un accessoire vendu seulement AVEC un produit n'a pas sa place dans les
    // rayons : il reste joignable par son adresse, pour qu'un lien de devis ou
    // de commande ne meure pas, et reste proposé sur les fiches qui le citent.
    accessoireSeul: false,
    gamme: {
      publie: true,
  ...
  const vitrines = await prisma.produitVitrine.findMany({
    ...
    include: {
      ...
      // La mise en avant : rang de la carte dans sa catégorie et dans son
      // rayon, choisi dans l'admin. Le tri par nom la place en tête.
      misesEnAvant: { select: { categorieId: true, sousCategorieId: true, ordre: true } },
    },
  });
  ...
  let cartes = vitrines.map((v) => {
    ...
    return {
      ...
      categorieSlug: slugCategoriePrincipale(v.categories, v.categoriePrincipaleId),
      sousCategorieSlug: slugSousCategoriePrincipale(v.sousCategories, v.sousCategoriePrincipaleId),
      avantCatalogue: v.misesEnAvant.find((m) => !m.categorieId && !m.sousCategorieId)?.ordre ?? null,
      avantCategorie: v.misesEnAvant.find((m) => m.categorieId)?.ordre ?? null,
      avantRayon: v.misesEnAvant.find((m) => m.sousCategorieId)?.ordre ?? null,
  ...
  const parMiseEnAvant = trieurMiseEnAvant({ categorieSlug, sousCategorieSlug, parNom });
  if (tri === "prix-asc") {
    cartes.sort((a, b) => {
