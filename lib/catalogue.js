...
// Le modèle Produit — l'ancien système, encore utilisé par une partie du
// catalogue — nomme ses champs autrement que les déclinaisons :
// prixPublicHT joue le rôle du tarif fournisseur, prixVenteHT celui du
// prix de vente. Sans cette conversion, ses prix ne suivaient pas la
// marge des Réglages : la carte affichait le nouveau montant tandis que
// la fiche restait sur l'ancien.
function resoudrePrixProduits(produits, marge) {
  return (Array.isArray(produits) ? produits : []).map((p) => ({
    ...p,
    prixVenteHT: prixVenteEffectif(
      {
        prixTarifHT: p.prixPublicHT,
        prixVenteHT: p.prixVenteHT,
        prixVerrouille: p.prixVerrouille,
      },
      marge
    ),
  }));
...
// Passe une vitrine brute (telle que sortie de Prisma) en vitrine « prête pour
// le prix » : produits et déclinaisons résolus via la marge. À appeler avant
// tout calculerPrixMini — sinon on retombe sur les montants stockés, qui ne
// suivent pas la marge des Réglages.
export function resoudreVitrinePourPrix(vitrine, marge) {
  return {
    ...vitrine,
    produits: resoudrePrixProduits(vitrine.produits, marge),
    declinaisons: resoudrePrixDeclinaisons(vitrine.declinaisons, marge),
  };
}
...
// Prix de vente d'une vitrine en mode « prix unique ».
//
// Une minoration porte un montant négatif : le fournisseur déduit une somme
// fixe, on déduit la même. La marge ne s'y applique donc pas — la multiplier
// creuserait la remise au-delà de ce que le fournisseur consent.
export function prixUnitaireEffectif(vitrine, marge) {
  ...
  if (tarif != null && tarif !== 0) {
    return tarif < 0 ? tarif : Math.round(tarif * (1 + (marge ?? 0)) * 100) / 100;
  }
...
// Attend des produits et des déclinaisons DÉJÀ résolus — passer des
// valeurs brutes donnerait un prix qui ne suit pas la marge.
...
export async function prixDepuisBase(lignes) {
  ...
  const vitrines = await prisma.produitVitrine.findMany({
    where: { id: { in: ids } },
    include: { gamme: { select: { venteSurDevis: true } } },
  });
  ...
  for (const l of lignes) {
    ...
    if (!v) {
      resultats.set(cle, { prixHT: null, surDevis: false, motif: "vitrine introuvable" });
      continue;
    }
    ...
    if (surDevis) {
      ...
      resultats.set(cle, { prixHT: v.prixAPartir ?? null, surDevis: true, motif: null });
      continue;
    }

    let base = null;
    let motif = null;
    if (v.sansDeclinaisons || !l.declinaisonId) {
      base = prixUnitaireEffectif(v, marge);
      if (base == null) motif = "aucun prix unique en base";
    } else {
      const decls = Array.isArray(v.declinaisons) ? v.declinaisons : [];
      const d = decls.find((x) => x.id === l.declinaisonId);
      if (!d) motif = "déclinaison disparue du catalogue";
      else base = prixVenteEffectif(d, marge);
    }

    const prixHT = base == null ? null : appliquerPromoVitrine(v, base).prixFinal;
    resultats.set(cle, { prixHT, surDevis: false, motif });
  }
...
function accessoireVersOption(a, marge) {
  ...
  // Les minorations portent un montant négatif : le client retire une pièce
  // et le devis la déduit. On teste donc que le prix existe, pas qu'il soit
  // positif — sinon les minorations du catalogue seraient toutes écartées.
  //
  // La marge ne s'applique pas à un montant négatif : le fournisseur déduit
  // une somme fixe, on déduit la même. La multiplier creuserait la remise
  // au-delà de ce que le fournisseur consent.
  let prixFixe = null;
  if (a.sansDeclinaisons) {
    const vente = Number(a.prixUnitaireHT);
    const venteOk = !Number.isNaN(vente) && vente !== 0;

    if (a.prixUnitaireVerrouille && venteOk) {
      prixFixe = vente;
    } else {
      const tarif = Number(a.prixUnitaireTarifHT);
      const tarifOk = !Number.isNaN(tarif) && tarif !== 0;

      if (tarifOk) {
        prixFixe = tarif < 0 ? tarif : Math.round(tarif * (1 + marge) * 100) / 100;
      } else if (venteOk) {
        prixFixe = vente;
      }
    }
...
export async function getCarteFront(gammeSlug, carteSlug) {
    ...
    include: {
      produits: {
        ...
        select: {
          ...
          prixPublicHT: true,
          prixVenteHT: true,
          // Sans ce champ, impossible de savoir si le prix est figé ou
          // doit suivre la marge des Réglages.
          prixVerrouille: true,
          longueur: true,
          hauteur: true,
          profondeur: true,
  ...
  const declinaisonsResolues = sansDeclinaisons ? [] : resoudrePrixDeclinaisons(vitrine.declinaisons, marge);
  const declinaisons = declinaisonsPubliques(declinaisonsResolues);
  // Les produits de l'ancien système passent par la même résolution : la
  // fiche affichait sinon un prix figé pendant que la carte suivait la
  // marge, d'où deux montants différents pour le même article.
  const produitsResolus = resoudrePrixProduits(vitrine.produits, marge);
  ...
  const prixMini = calculerPrixMini(
    { ...vitrine, produits: produitsResolus, declinaisons: declinaisonsResolues },
    surDevis,
    marge
  );
  ...
  return {
    ...
    carte: {
      ...
      declinaisons,
      finitionsProduit,
      produits: produitsResolus.map((p) => ({
