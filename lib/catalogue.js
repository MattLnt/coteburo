...
// le prix » : déclinaisons résolues via la marge. À appeler avant tout
// calculerPrixMini — sinon on retombe sur les montants stockés, qui ne suivent
// pas la marge des Réglages.
...
export function calculerPrixMini(vitrine, surDevis, marge = null) {
  ...
  return prixDeclinaisons.length ? Math.min(...prixDeclinaisons) : null;
}
...
export async function getCarteFront(gammeSlug, carteSlug) {
  ...
  const prixMini = calculerPrixMini(
    { ...vitrine, declinaisons: declinaisonsResolues },
    surDevis,
    marge
  );
