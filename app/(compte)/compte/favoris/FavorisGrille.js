...
export default function FavorisGrille({ produits }) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [liste, setListe] = useState(produits);

  const retirer = (codeRacine) => {
    // Optimistic : on retire de la liste tout de suite
    setListe((l) => l.filter((p) => p.codeRacine !== codeRacine));
    startTransition(async () => {
      await retirerFavori(codeRacine);
      router.refresh();
    });
  };
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
      {liste.map((p) => {
        ...
        return (
          <div key={p.codeRacine} className="group rounded-2xl border border-line bg-surface overflow-hidden hover:shadow-[0_20px_50px_-30px_rgba(33,36,40,0.3)] transition">
            <Link href={p.slug ? `/produit/${p.slug}` : "#"} className="block">
              <div className="aspect-[4/3] bg-surface-2 overflow-hidden relative">
                {p.imageUrl ? (
                  <img src={p.imageUrl} alt={p.designation} className="w-full h-full object-cover group-hover:scale-105 transition duration-500" />
                ) : (
            ...
            <div className="p-5">
              {p.gamme && <p className="text-[11px] font-bold uppercase tracking-wide text-orange">{p.gamme}</p>}
              <Link href={p.slug ? `/produit/${p.slug}` : "#"}>
                <h3 className="font-display font-bold text-[15px] text-ink mt-1 leading-snug line-clamp-2 group-hover:text-orange transition min-h-[42px]">{p.designation}</h3>
              </Link>
              <div className="flex items-center justify-between mt-3">
                <p className="font-display font-bold text-lg text-ink">
                  {p.prixVarieSelonFinition && <span className="text-[12px] font-normal text-ink-soft">dès </span>}
                  {euro(prix)}<span className="text-[12px] font-normal text-ink-soft"> HT</span>
                </p>
                <button
                  onClick={() => retirer(p.codeRacine)}
                  disabled={isPending}
