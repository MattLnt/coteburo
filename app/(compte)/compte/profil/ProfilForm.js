...
const labelStyle = "block text-[12.5px] sm:text-[13px] font-semibold mb-1.5 text-ink";

const CRITERES = [
  { label: "9 car. min.", labelLong: "9 caractères min.", test: (p) => p.length >= 9 },
  { label: "Majuscule", labelLong: "Une majuscule", test: (p) => /[A-Z]/.test(p) },
  { label: "Minuscule", labelLong: "Une minuscule", test: (p) => /[a-z]/.test(p) },
  { label: "Chiffre", labelLong: "Un chiffre", test: (p) => /[0-9]/.test(p) },
  { label: "Symbole", labelLong: "Un symbole", test: (p) => /[^A-Za-z0-9]/.test(p) },
];

// Titre de carte — l'icône de 44px prenait de la hauteur sans rien apporter.
function CardHead({ titre, sousTitre }) {
  return (
    <div className="mb-4 sm:mb-5">
      <h2 className="font-display font-bold text-[15.5px] sm:text-lg text-ink leading-tight">{titre}</h2>
      <p className="text-[11.5px] sm:text-[13px] text-ink-soft mt-0.5">{sousTitre}</p>
    </div>
  );
}
...
export default function ProfilForm({ prenom, nom, email }) {
  ...
  const Msg = ({ m }) => m ? (
    <p className={`text-[13px] rounded-lg px-3 py-2.5 mt-4 flex items-center gap-2 ${m.type === "ok" ? "bg-[#e8f6f0] text-[#1f7a52]" : "bg-orange-tint text-orange-dark"}`}>
      {m.type === "ok" ? <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="shrink-0"><path d="M20 6 9 17l-5-5" /></svg> : <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" className="shrink-0"><circle cx="12" cy="12" r="10" /><path d="M12 8v4M12 16h.01" /></svg>}
      {m.txt}
    </p>
  ) : null;
  ...
  return (
    <div className="flex flex-col gap-3 sm:gap-6">
      {/* Carte identité */}
      <div className="rounded-[18px] sm:rounded-[24px] p-[18px] sm:p-8 relative overflow-hidden" style={{ background: "linear-gradient(150deg, #23262a 0%, #2d2620 60%, #3a2820 100%)" }}>
        ...
        <div className="relative flex items-center gap-3.5 sm:gap-5">
          <span className="grid place-items-center w-[54px] h-[54px] sm:w-16 sm:h-16 rounded-full bg-orange text-white font-display font-bold text-xl sm:text-2xl shrink-0">{initiale}</span>
          <div className="min-w-0">
            <p className="font-display font-bold text-[18px] sm:text-2xl text-white truncate">{[prenom, nom].filter(Boolean).join(" ") || "Mon compte"}</p>
            <p className="text-white/60 text-[12px] sm:text-sm mt-0.5 truncate">{email}</p>
          </div>
        </div>
      </div>

      {/* Deux cartes */}
      <div className="grid lg:grid-cols-2 gap-3 sm:gap-6 items-start">
        {/* Infos personnelles */}
        <div className="rounded-[18px] sm:rounded-[24px] border border-line bg-surface p-4 sm:p-7">
          <CardHead titre="Informations personnelles" sousTitre="Votre nom sur vos commandes" />
          <div className="flex flex-col gap-3 sm:gap-4">
            ...
            <div>
              ...
              <p className="text-[11px] sm:text-[12px] text-ink-soft mt-1.5">L&apos;email ne peut pas être modifié.</p>
            </div>
          </div>
          <Msg m={infosMsg} />
          <button onClick={saveInfos} disabled={infosLoad} className="w-full sm:w-auto rounded-full bg-orange text-white font-semibold px-6 py-3 mt-4 sm:mt-5 text-[13.5px] sm:text-base hover:bg-orange-dark transition disabled:opacity-60">
            {infosLoad ? "Enregistrement…" : "Enregistrer"}
          </button>
        </div>

        {/* Mot de passe */}
        <div className="rounded-[18px] sm:rounded-[24px] border border-line bg-surface p-4 sm:p-7">
          <CardHead titre="Sécurité" sousTitre="Modifiez votre mot de passe régulièrement" />
          <div className="flex flex-col gap-3 sm:gap-4">
            <div>
            ...
            <div>
              ...
              {/* Pastilles plutôt qu'une grille de coches : plus lisible sur
                  un écran étroit, et l'état se lit d'un coup d'œil. */}
              {pwd.nouveau.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {CRITERES.map((c, i) => {
                    const ok = c.test(pwd.nouveau);
                    return (
                      <span key={i} className={`inline-flex items-center gap-1 text-[11px] px-2.5 py-1 rounded-full font-medium ${ok ? "bg-orange-tint text-orange-dark" : "bg-surface-2 text-ink-soft"}`}>
          ...
          <Msg m={pwdMsg} />
          <button onClick={savePwd} disabled={pwdLoad || !pwdValide || !pwd.actuel} className="w-full sm:w-auto rounded-full bg-orange text-white font-semibold px-6 py-3 mt-4 sm:mt-5 text-[13.5px] sm:text-base hover:bg-orange-dark transition disabled:opacity-50 disabled:cursor-not-allowed">
