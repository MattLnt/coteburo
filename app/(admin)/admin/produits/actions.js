    ...
    <div>
      ...
      <div style={{ ...card, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <span style={{ fontSize: 13.5, color: "#5c616a" }}>
          🎨 Réutilise des palettes déjà définies (couleurs + swatches) sans les ressaisir.
        </span>
...
// ─────────── Sélecteur de PALETTES depuis la bibliothèque partagée ───────────
// On coche des palettes entières (fiable et rapide). Tout est clé par id unique
// (palette + finition) → aucune sélection en miroir possible.
function ModaleBibliotheque({ onClose, onImported, vitrineId, groupeId = null }) {
  const [modeles, setModeles] = useState(null);
  const [selection, setSelection] = useState([]); // ids de finitions (uniques)
  const [groupeNom, setGroupeNom] = useState("");
  const [recherche, setRecherche] = useState("");
  const [detail, setDetail] = useState({}); // paletteKey -> déplié (choix fin des coloris)
  const [isPending, startTransition] = useTransition();
  const [erreur, setErreur] = useState("");

  useEffect(() => { getFinitionsModeles().then(setModeles); }, []);

  // Regroupe par palette — clé = id UNIQUE de palette (repli sur le nom si absent).
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 250, display: "grid", placeItems: "center", padding: 16 }}>
      ...
      <div style={{ position: "relative", width: "100%", maxWidth: 620, maxHeight: "85vh", overflowY: "auto", background: "#fff", border: "1px solid #ece8e0", borderRadius: 20, padding: 26 }}>
        ...
        <p style={{ fontSize: 13, color: "#9aa0a8", margin: "0 0 18px" }}>Coche une palette entière — toutes ses couleurs sont ajoutées d'un coup. Déplie une palette si tu veux choisir des coloris précis.</p>
        ...
        {modeles && palettes.length === 0 && (
          <div style={{ padding: 20, textAlign: "center", color: "#9aa0a8", fontSize: 13.5 }}>
            La bibliothèque est vide. Crée d'abord des palettes dans Architecture → onglet Finitions.
          </div>
        )}
        ...
        {palettesFiltrees.map((p) => {
          ...
          return (
            <div key={p.key} style={{ border: "1.5px solid " + (etat === "aucune" ? "#ece8e0" : "#f0661b"), borderRadius: 14, marginBottom: 10, overflow: "hidden", background: etat === "aucune" ? "#fff" : "#fef7f2" }}>
              {/* En-tête palette = case à cocher globale */}
              <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px" }}>
                <button type="button" onClick={() => togglePalette(p)}
                  style={{ width: 22, height: 22, borderRadius: 6, flexShrink: 0, cursor: "pointer",
                    border: "2px solid " + (etat === "aucune" ? "#cfc9bd" : "#f0661b"),
                    background: etat === "toutes" ? "#f0661b" : etat === "partielle" ? "#f7c8ab" : "#fff",
                    color: "#fff", display: "grid", placeItems: "center", fontSize: 13, fontWeight: 800, lineHeight: 1 }}>
              ...
              {/* Détail : choix fin des coloris (clé unique par finition) */}
              {deplie && (
                  ...
                  {p.items.map((m) => {
                    ...
                    return (
                      <button key={`${p.key}::${m.id}`} onClick={() => toggleFinition(m.id)} type="button"
                        style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 9px", borderRadius: 9, cursor: "pointer", textAlign: "left",
                          border: "1.5px solid " + (actif ? "#f0661b" : "#ece8e0"), background: actif ? "#fff" : "#faf8f4" }}>
