...
export default async function ContactPage() {
  ...
  return (
    <main>
      ...
      {/* Bandeau infos : 4 colonnes égales pleine largeur */}
      <section className="mx-auto max-w-[1400px] px-5 sm:px-7 pb-6">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {INFOS.map((it) => (
            <div key={it.label} className="rounded-2xl border border-line bg-surface p-5">
              ...
              <p className="text-[12px] font-semibold uppercase tracking-wide text-ink-soft">{it.label}</p>
              {it.href ? (
                <a href={it.href} className="block font-display font-bold text-[15px] mt-1 hover:text-orange transition whitespace-pre-line leading-snug">{it.value}</a>
              ) : (
                <p className="font-display font-bold text-[15px] mt-1 whitespace-pre-line leading-snug">{it.value}</p>
              )}
            </div>
          ))}
        </div>
      ...
      {/* Formulaire (gauche) + carte (droite), alignés en haut, hauteurs naturelles */}
      <section className="mx-auto max-w-[1400px] px-5 sm:px-7 pb-20">
        <div className="grid lg:grid-cols-[1.3fr_1fr] gap-6 items-start">
          <ContactForm />

          <div className="rounded-[24px] overflow-hidden border border-line lg:sticky lg:top-24">
            <iframe
