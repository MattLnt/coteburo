    ...
    <main>
      <div className="mx-auto max-w-[1440px] px-5 sm:px-7">
        ...
        {realisations.length === 0 ? (
          <div className="rounded-2xl border border-line bg-surface p-8 sm:p-12 text-center mb-14 sm:mb-20">
            <p className="text-ink-soft text-[13.5px] sm:text-base">Nos réalisations seront bientôt présentées ici.</p>
            <Link href="/devis" className="inline-flex items-center gap-2 rounded-full bg-orange text-white font-semibold px-6 py-3 mt-5 text-[13.5px] sm:text-base hover:bg-orange-dark transition">Demander un devis →</Link>
          </div>
        ) : (
            ...
            return (
              <div key={r.id} className="grid lg:grid-cols-2 items-center gap-5 lg:gap-16 py-7 sm:py-16 lg:py-20 border-t border-line">
                ...
                >
                  ...
                  {/* Le survol n'existe pas au doigt : la pastille reste visible sur mobile */}
                  <span className="absolute bottom-3.5 right-3.5 sm:bottom-5 sm:right-5 inline-flex items-center gap-2 rounded-full bg-white/95 text-ink text-[11.5px] sm:text-[13px] font-bold px-3.5 sm:px-5 py-2 sm:py-2.5 sm:opacity-0 sm:translate-y-1.5 transition duration-300 sm:group-hover:opacity-100 sm:group-hover:translate-y-0">
                    Voir le projet
                ...
                <div className={inverse ? "lg:order-1" : ""}>
                  <p className="font-display font-bold text-[12.5px] sm:text-[15px] text-orange flex items-center gap-3 mb-2.5 sm:mb-5">
                  ...
                  {(r.client || r.surface) && (
                    <div className="flex flex-wrap gap-2 mb-3.5 sm:mb-6">
                      {r.client && <span className="text-[11px] sm:text-[12px] font-semibold rounded-full bg-orange-tint text-orange-dark px-3 sm:px-3.5 py-1.5">{r.client}</span>}
                      {r.surface && <span className="text-[11px] sm:text-[12px] font-semibold rounded-full border border-line text-ink-soft px-3 sm:px-3.5 py-1.5">{r.surface}</span>}
                    </div>
                  )}
        ...
        {/* CTA final */}
        <div className="relative overflow-hidden rounded-[20px] sm:rounded-[28px] bg-charcoal text-center px-6 sm:px-14 py-10 sm:py-[72px] mt-4 mb-12 sm:mb-20">
          ...
          <p className="relative text-[10px] sm:text-[11.5px] font-bold uppercase tracking-[0.2em] text-[#ffb98a]">Vous avez un projet ?</p>
          <h2 className="relative font-display font-bold text-white text-[26px] sm:text-[42px] mt-2.5 sm:mt-4 max-w-[640px] mx-auto leading-tight">Parlons-en autour d&apos;un café.</h2>
          <p className="relative text-[#c4c9d0] text-[13px] sm:text-[16px] mt-3 sm:mt-4 max-w-[520px] mx-auto leading-relaxed">
            Nos experts vous accompagnent sur l&apos;aménagement de vos bureaux — d&apos;un poste isolé à plusieurs centaines de m².
          </p>
          <Link href="/devis" className="relative inline-flex items-center gap-2.5 mt-6 sm:mt-8 bg-orange text-white font-bold text-[13.5px] sm:text-[15px] px-7 sm:px-8 py-3.5 sm:py-4 rounded-full hover:bg-orange-dark transition">
            Demander un devis →
          </Link>
        </div>
      </div>
